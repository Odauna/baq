...
import { saveUpPembagianBeras } from "@/lib/zakat/pembagian"
...
export function DialogEditPembagianBeras({
    bid,
    banggota,
    bkondisi,
    bnominal
    ...
    bid: string
    banggota: number
    bkondisi: 'Sama dengan' | 'Lebih dari sama dengan' | 'Kurang dari sama dengan'
    bnominal: number
}) {
    ...
    const boundSaveUpPembagianBeras = saveUpPembagianBeras.bind(null, bid)
    const {executeAsync, result} = useAction(boundSaveUpPembagianBeras)
    ...
    return (
        <DialogContent>
            <DialogHeader>
                <DialogTitle>Formulir Ubah Kondisi Pembagian Zakat Fitrah Beras per Jumlah Anggota Keluarga</DialogTitle>
